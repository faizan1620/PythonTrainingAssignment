...
def test_post_book():
    global id
    response = client.post("/book/", json = requestBody, params={"student_id": student_id})
    id = response.json()["id"]
    assert response.status_code == 200
...
def test_update_book_by_id():
    ...
    assert response.status_code == 200
    assert response.json() == {'title': 'updated_book', 'department': 'updated_department'}
...
def test_delete_book_by_id_doesnot_exist():
    response = client.delete(f"/book/1000")
    assert response.status_code == 404
