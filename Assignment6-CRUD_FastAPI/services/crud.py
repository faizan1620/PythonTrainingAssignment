...
def create_student(db: Session, student: schema.Student) -> schema.Student:
    try:
        db_student = models.Student(name=student.name,branch=student.branch,age=student.age,gender=student.gender,registration=student.registration)
        db.add(db_student)
        db.commit()
...
def update_student(db: Session, student: schema.Student, student_id:int) -> schema.Student :
    try:
        db.query(models.Student).filter(models.Student.id == student_id).update({ models.Student.name: student.name,models.Student.branch:student.branch,models.Student.age:student.age,models.Student.gender:student.gender,models.Student.registration:student.registration })
        db.commit()
...
def delete_student(db: Session, student_id:int) -> schema.Student:
    try:
        db_student = db.query(models.Student).filter(models.Student.id == student_id).delete()
        db.commit()
        return db_student
    except Exception as e:
         raise HTTPException(400,str(e))
...
def create_student_book(db: Session, book: schema.Book, student_id: int) -> schema.Student:
    try:
...
def update_book(db: Session, book: schema.Book, book_id:int) -> schema.Student :
    try:
        db_book = db.query(models.Book).filter(models.Book.id == book_id).update({**book.dict()})
        db.commit()
        return db_book
    except Exception as e:
       raise HTTPException(400,str(e))
